...
void inputManager(unsigned int pin0, unsigned int pin1, unsigned int pin2) {
  ...
  int p[3];
  p[0] = digitalRead(pin0);
  p[1] = digitalRead(pin1);
  p[2] = digitalRead(pin2);
  ...
  if (player_state != P_DEAD) {
   ...
   playertimer++; 
    ...
    if (playertimer > 1000/75) { 
          ...
          if (life > 0) {  
            player_position.x = (int)((last_safe_position.x/8)*8);/*+(8*-ultima_directie);*/
